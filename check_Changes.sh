...
checkProcess(){
    ...
    processRunningA=($processRunning)

    Array3=()
    for i in "${processRunningA[@]}"; do
        skip=
        for j in "${processCheck[@]}"; do
            [[ $i == $j ]] && { skip=1; break; }
        done
        [[ -n $skip ]] || Array3+=("$i")
    done
    ...
    if [ ${#Array3[@]} -gt 0 ]; then
        ...
        echo -e "${GREEN}No non Standard Processes running. ${NC}"
    fi
}
